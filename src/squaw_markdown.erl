...
-module(squaw_markdown).

-export([conv/1,
         conv_utf8/1,
         collect_meta_conv_file/2]).
...
collect_meta_conv_file(FileIn, FileOut) ->
    case file:open(FileIn, [read, {encoding, utf8}]) of
        {ok, Device} -> 
                        H = io:get_line(Device,''),
                        {meta, Terms} = list_to_term(H),
                        Input = get_rest_lines(Device,[]),
                        Output = conv(Input),
...
get_rest_lines(Device, Accum) ->
    case io:get_line(Device,'') of
        eof  -> file:close(Device),
                Accum;
        Line ->
            get_rest_lines(Device,Accum ++ Line) %% OPTIMIZE
    end.
...
htmlchars1([$*, $* | T], A)          -> {T2, NewA} = strong(T, $*),
                                        htmlchars1(T2, [NewA | A]);
%% N.B. new: handle abbreviations PLUS allow escaped
htmlchars1([$\\, $*, $[ | T], A)     -> htmlchars1(T, [$[, $* | A]);
htmlchars1([$*, $[ | T], A)          -> {T2, NewA} = abbreviation(T, $]),
                                        htmlchars1(T2, [NewA | A]);
%% likewise for strong
htmlchars1([$\\, $* | T], A)         -> htmlchars1(T, [$* | A]);
...
htmlchars1([$_ | T], A)              -> {T2, NewA} = emphasis(T, $_),
                                        htmlchars1(T2, [NewA | A]);
%% N.B. new: a la pandoc -S | --smart
htmlchars1([$., $., $. | T], A)      -> htmlchars1(T, ["…" | A]); % &hellip;
htmlchars1([$-, $- | T], A)          -> htmlchars1(T, ["—" | A]); % &mdash;
htmlchars1([$`, $` | T], A)          -> htmlchars1(T, ["“" | A]); % &ldquo;
htmlchars1([$\', $\' | T], A)        -> htmlchars1(T, ["”" | A]); % &rdquo;

%% N.B. new: default single quotes PLUS allow \` & \' to maintain intact
htmlchars1([$\\, $\' | T], A)        -> htmlchars1(T, [$\' | A]);
htmlchars1([$\' | T], A)             -> htmlchars1(T, ["’" | A]); % &rsquo;
htmlchars1([$\\, $` | T], A)         -> htmlchars1(T, [$` | A]);
htmlchars1([$` | T], A)              -> htmlchars1(T, ["‘" | A]); % &lsquo;

%% handle emacs ~ .org in-line code PLUS escaping
htmlchars1([$\\, $= | T], A)         -> htmlchars1(T, [$= | A]);
htmlchars1([$= | T], A)              -> {T2, NewA} = code(T, $=),
                                        htmlchars1(T2, [NewA | A]);
%% handle backtick PLUS escaping
%htmlchars1([$\\, $` | T], A)         -> htmlchars1(T, [$` | A]);
...
abbreviation(List, Delim) -> interpolate(List, Delim, "abbr", "" ,[]).
code(List, Delim)         -> interpolate(List, Delim, "code", "" ,[]).
...
-include("squaw_markdown_tests.hrl").
